...
int main() {
  ...
  // start in lane 1 with reference velocity of 0 mph
  int lane = 1;
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // calculate variables outside the for loop to save calculation time
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            check_car_s += ((double)previousSize * .02 * speedChecker); // if using previous points can project s value out
          ...
          // use the previous path's end point as starting reference
            ...
            double ref_x_prev = previous_path_x[previousSize-2];
            double ref_y_prev = previous_path_y[previousSize-2];
          ...
          for (int i = 1; i <= 50 - previous_path_x.size(); i++) {
            double N = (target_dist / (.02 * referenceVelocity / 2.24));
